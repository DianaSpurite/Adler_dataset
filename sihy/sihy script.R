...
surv        <- subset( sit_hys, !is.na( survives) ) %>%
  subset( size_t0 != 0 ) %>%
...
write.csv( surv, "data/sit_hys/survival_df.csv" )
write.csv( grow, "data/sit_hys/growth_df.csv" )
write.csv( recr, "data/sit_hys/recruitment_df.csv" )
...
# png( 'results/sit_hys/histograms.png', width = 8, height = 3, units = "in", res = 150 )

par( mfrow = c( 1, 2 ), mar = c( 3.5, 3.5, 1, 0.2 ), mgp = c( 2, 0.7, 0 ), cex = 0.8 )

hist( sit_hys$size_t0, main = "Histogram of size at time t0", xlab = "Size at time t0" )
hist( sit_hys$size_t1, main = "Histogram of size at time t1", xlab = "Size at time t1" )



par( mfrow = c( 1, 2 ), mar = c( 3.5, 3.5, 1, 0.2 ), mgp = c( 2, 0.7, 0 ), cex = 0.8 )

hist( sit_hys$logsize_t0, main = "Histogram of log-transformed size at time t0", xlab = "log( Size at time t0 )" )
hist( log( sit_hys$size_t1 ), main = "Histogram of log-transformed size at time t1", xlab = "log( Size at time t1 )" )



h    <- ( max( sit_hys$logsize_t0, na.rm = T ) - min( sit_hys$logsize_t0, na.rm = T ) ) / 200
lwr  <- min( sit_hys$logsize_t0, na.rm = T ) + ( h * c( 0:( 200 - 1 ) ) )
upr  <- lwr + h
mid  <- lwr + ( 1/2 * h )

binned_prop <- function( lwr_x, upr_x, response ){
  
  id  <- which( sit_hys$logsize_t0 > lwr_x & sit_hys$logsize_t0 < upr_x ) 
  tmp <- sit_hys[id,]
...
write.csv( grow_out, "data/sit_hys/grow_pars.csv", row.names = F )
...
write.csv( surv_out, "data/sit_hys/surv_pars.csv", row.names = F )



recSize <- sit_hys %>% subset( recruit == 1)
...
write.csv( others, "data/sit_hys/other_pars.csv", row.names = F )
...
write.csv( pars_mean, "data/sit_hys/pars_mean.csv", row.names = F )
